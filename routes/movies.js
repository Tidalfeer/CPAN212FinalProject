...
router.post('/add', requireLogin, upload.single('poster'), [
  ...
  const data = req.body;
  if (!errors.isEmpty()) {
    return res.status(422).render('movies/add', { errors: errors.array(), data, title: 'Add Movie' });
  }
...
router.put('/:id', requireLogin, requireOwner, upload.single('poster'), [
  ...
  const errors = validationResult(req);
  if (!errors.isEmpty()) {
    return res.status(422).render('movies/edit', { errors: errors.array(), data: { ...req.body, _id: req.params.id }, title: 'Edit Movie' });
  }
  const genres = (req.body.genres || '').split(',').map(s => s.trim()).filter(Boolean);
  try {
...
router.post('/:id/comment', requireLogin, async (req, res) => {
  try {
    const movie = await Movie.findById(req.params.id);
    movie.comments.push({ user: req.session.user.username, text: req.body.comment });
    await movie.save();
    res.redirect('/movies/' + req.params.id);
  } catch (e) {
    console.error(e);
